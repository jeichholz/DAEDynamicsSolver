function [systemOfODEs,postProcessingEqns]=generateGoverningDEs(DEs,positionConstraints,velocityConstraints,...
    stateVariables,options)
...
parameters=options.parameters;
    ...
    parameters=helpers.evalSymfun2Expr(parameters);
    ...
    [system,funsToVars,varsToFuns,order,baseFunctions]=helpers.symFunsToSymVars(system);

    if ~isempty(parameters)
        parameters=helpers.symFunsToSymVars(parameters);
    end
    ...
    for i=1:numel(unknownFunctions)
       ...
       if  order(helpers.symFunsToSymVars(unknownFunctions(i)))==0
            dfprintf('(force)');
       end
    ...
    if exist('stateVariables','var') && ~isempty(stateVariables)
        stateVariables=helpers.evalSymfun2Expr(stateVariables);
        ...
        soln=solveInTermsOf(system,helpers.symFunsToSymVars(stateVariables));
        
        %Which equations to grab?  All of the ones that are the highest
        %derivative of a state function. 
        solveInTermsOfBase=unique(baseFunctions(helpers.symFunsToSymVars(stateVariables)));
