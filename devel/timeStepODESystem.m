...
PostProcessFunctions=options.PostProcessFunctions;
    ...
    ODESystem=helpers.evalSymfun2Expr(ODESystem);
    InitialConditions=helpers.evalSymfun2Expr(InitialConditions);

    ICstruct=helpers.parseConditions(InitialConditions);

    [firstOrderSystem,S]=odeToVectorField(ODESystem);

    [~,~,v2f,~,baseFunctions]=helpers.symFunsToSymVars(ODESystem);
    ...
    Y0=helpers.structToVec(ICstruct,varToIdx);

    opts=odeset(AbsTol=options.AbsTol,RelTol=options.RelTol);

    %Now solve. 
    [t,ys]=ode23s(matlabFunction(firstOrderSystem,'vars',{'t','Y'}),tspan,Y0,opts);
    ...
    DEs=helpers.symFunsToSymVars(ODESystem);
    for i=1:numel(DEs)
        LH=lhs(DEs(i));
    ...
    if exist("PostProcessFunctions","var") && ~isempty(PostProcessFunctions)
        PP=helpers.symFunsToSymVars(PostProcessFunctions);
        for i=1:numel(PP)
            LH=lhs(PP(i));
    ...
    function symToIdx=createVarToIndexMap(vars,S)
        [~,~,varsToFuns,~,baseVars]=helpers.symFunsToSymVars(vars);
