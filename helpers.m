classdef helpers
    ...
    methods (Static)
        function ICstruct=parseConditions(ics)
            ...
            for i=1:numel(ics)
                ...
                elseif class(ics{i})=="char"
                    ...
                    [~,asSymbol]=helpers.symFunsToSymVars(ics{i});
                    tmp=lhs(ics{i});
        ...
        function [soln]=timeStepDAESystem(DEs,PositionConstraints,VelocityConstraints,tspan,ics,options)
            ...
            %expressions.
            DEs=helpers.evalSymfun2Expr(DEs);
            PositionConstraints=helpers.evalSymfun2Expr(PositionConstraints);
            VelocityConstraints=helpers.evalSymfun2Expr(VelocityConstraints);

            ICstruct=helpers.parseConditions(ics);

            if exist("hints","var") && ~isempty(hints)
                hintsStruct=helpers.parseConditions(hints);
            else
                hintsStruct=[];
            end
            ...
            [~,~,varsToSyms,diffOrdersOriginal,baseVars]=helpers.symFunsToSymVars([DEs;constraints]);

            constraints=helpers.symFunsToSymVars(constraints);
            ...
            %rid of that.
            [firstOrderVars,firstOrderFuns2FirstOrderVars]=helpers.symFunsToSymVars(firstOrderFuns);
            [~,~,~,diffOrders]=helpers.symFunsToSymVars(firstOrderSystem);
            ...
            if ~isempty(constraints)
                ...
                specificConstraints=helpers.structsubs(specificConstraints,ICstruct);
            ...
            [Y0,Y0Fixed]=helpers.structToVec({ICsoln,ICstruct},firstOrderVarToIdx);
            ...
            M=helpers.symFunsToSymVars(Msym);
            ...
            F=helpers.symFunsToSymVars(Fsym);
            ...
            %Ok, solve!
            opt = odeset('Mass', M,'RelTol', options.RelTol, 'AbsTol' , options.AbsTol,'InitialSlope',YP0);
